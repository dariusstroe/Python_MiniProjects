...
def get_words_from_text(text_path):
    with open(text_path,"r") as f:
        text=f.read()
        text = re.sub(r'\[(.+)\]', ' ', text)
        text=" ".join(text.split()) # turn whitespace into a single space
        text=text.lower()
...
def make_graph(words):
    ...
    for word in words:
        #check for word to be in the graph
...
def main():
    ...
    print(" ".join(composition)) #return string where all the words are separated by a space
